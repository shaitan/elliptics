...
struct type_traits<ioremap::elliptics::data_pointer> {
	...
	static inline void pack(msgpack::packer<Stream> &target, const ioremap::elliptics::data_pointer &source) {
		target.pack_raw(source.size());
		target.pack_raw_body(static_cast<const char *>(source.data()), source.size());
	}

	static inline void unpack(const msgpack::object &source, ioremap::elliptics::data_pointer &target) {
