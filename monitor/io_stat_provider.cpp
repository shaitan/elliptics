...
dump_states_stats(struct dnet_node *n, rapidjson::Value &value, rapidjson::Document::AllocatorType &allocator) {
...
void io_stat_provider::statistics(uint64_t categories,
	...
	value.AddMember("states", dump_states_stats(m_node, states, allocator), allocator);
	value.AddMember("blocked", m_node->io->blocked == 1, allocator);

	rapidjson::Value pools(rapidjson::kObjectType);
	dnet_io_pools_dump_stats(m_node, pools, allocator);
