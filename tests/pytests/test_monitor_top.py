...
def check_key_fields(keys):
    # all items in top key list must contain group_id, key_id,
    # traffic size generated by key access, key access frequency
    for k in keys:
        assert k['group']
        assert k['id']
        assert k['size']
...
def check_response_fields(response, config_params):
    # top object must contain top_result_limit, period_in_seconds fields conformed with server config values
    assert response['top']['top_result_limit'] == config_params['top_length']
    assert response['top']['period_in_seconds'] == config_params['top_period']
...
class TestMonitorTop:
    ...
    def test_single_write(self, cluster, client):
        '''
        ...
        top_keys = []
        for remote, port in zip(cluster.remotes, cluster.monitors):
            remote = remote.split(':')[0]
            response = get_top_by_http(remote, port)
            # check that response contains required fields
            check_response_fields(response, cluster.config_params)
        ...
        self.__check_key_existance_using_session_monitor(session, cluster.config_params, test_key)
    ...
    def __check_key_existance_using_session_monitor(self, session, config_params, test_key):
        ...
        for entry in entries:
            ...
            # check that response contains required fields
            check_response_fields(response, config_params)
            top_keys = response['top']['top_by_size']
            if has_key(test_key, top_keys):
                break
        # check that written key appears among top keys
