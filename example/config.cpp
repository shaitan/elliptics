...
extern "C" void dnet_destroy_config_data(struct dnet_node *node) {
	if (node->config_data)
		node->config_data->destroy_config_data(node->config_data);
}

static void dnet_config_data_destroy(dnet_config_data *public_data) {
...
extern "C" struct dnet_node *dnet_parse_config(const char *file, int mon) {
	...
	try {
		...
		if (data->remotes.size() != 0) {
			int err = dnet_add_state(node, reinterpret_cast<const dnet_addr *>(data->remotes.data()),
			                         data->remotes.size(), 0);
			if (err < 0)
...
static cache::cache_config parse_cache_config(const config_data &data, const kora::config_t &config) {
...
, pool_id{config.has("pool_id") ? config.at<std::string>("pool_id") : ""}
...
std::shared_ptr<kora::config_parser_t> config_data::parse_config() {
	...
	if (dnet_time_is_empty(&config_timestamp) || dnet_time_before(&config_timestamp, &ts)) {
		config_timestamp = ts;
		m_parser = std::make_shared<kora::config_parser_t>();
		m_parser->open(config_path);
		return m_parser;
	} else {
		return m_parser;
	}
