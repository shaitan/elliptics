...
static int blob_iterate_callback_common(struct eblob_disk_control *dc,
                                        int fd,
	...
	if (dc->flags & BLOB_DISK_CTL_EXTHDR) {
		...
		if (!no_meta || (dc->flags & BLOB_DISK_CTL_UNCOMMITTED)) {
			...
			if (!err) {
				...
				DNET_LOG_ERROR(c->blog,
				               "blob: iter: %s: dnet_ext_hdr_read failed: %d. Use empty extended "
				               "header for this key",
				               dnet_dump_id_str_full(dc->key.id), err);
		...
		if (ehdr.size) {
			...
			if (!err) {
				DNET_LOG_ERROR(c->blog, "blob: iter: %s: dnet_read_json_header failed: %d. Use empty "
				                        "json for this key",
				               dnet_dump_id_str_full(dc->key.id), err);
				err = 0;
			}
...
static int blob_write(struct eblob_backend_config *c, void *state, struct dnet_cmd *cmd, void *data) {
	...
	DNET_LOG_NOTICE(c->blog, "%s: EBLOB: blob-write: WRITE: start: %s", dnet_dump_id_str(io->id),
	                dnet_print_io(io));
	...
	if (io->flags & DNET_IO_FLAGS_PREPARE) {
		...
		if (err) {
			DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-write: eblob_write_prepare: size: %" PRIu64 ": %s %d",
			               dnet_dump_id_str(io->id), io->num + ehdr_size, strerror(-err), err);
			goto err_out_exit;
		}
		...
		if (err) {
			DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-write: eblob_plain_writev: header WRITE: %d: %s",
			               dnet_dump_id_str(io->id), err, strerror(-err));
			goto err_out_exit;
		}

		DNET_LOG_NOTICE(c->blog, "%s: EBLOB: blob-write: eblob_write_prepare: size: %" PRIu64 ": Ok",
	...
	if (io->size) {
		...
		if (err) {
			DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-write: WRITE: %d: %s", dnet_dump_id_str(io->id), err,
			               strerror(-err));
			goto err_out_exit;
		}

		DNET_LOG_NOTICE(c->blog, "%s: EBLOB: blob-write: WRITE: Ok: offset: %" PRIu64 ", size: %" PRIu64,
	...
	if (io->flags & DNET_IO_FLAGS_COMMIT) {
		...
		if (io->flags & DNET_IO_FLAGS_PLAIN_WRITE) {
			...
			if (err) {
				DNET_LOG_ERROR(c->blog,
				               "%s: EBLOB: blob-write: eblob_write_commit: size: %" PRIu64 ": %s %d",
				               dnet_dump_id_str(io->id), csize, strerror(-err), err);
				goto err_out_exit;
			}

			DNET_LOG_NOTICE(c->blog, "%s: EBLOB: blob-write: eblob_write_commit: size: %" PRIu64 ": Ok",
	...
	if (!err && wc.data_fd == -1) {
		...
		if (err) {
			DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-write: eblob_read: size: %" PRIu64 ": %s %d",
			               dnet_dump_id_str(io->id), io->num, strerror(-err), err);
			goto err_out_exit;
		}
	}
	...
	if (err) {
		DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-write: dnet_send_file_info: fd: %d, offset: %" PRIu64
		                        ", offset-within-fd: %" PRIu64 ", size: %" PRIu64 ": %s %d",
		               dnet_dump_id_str(io->id), wc.data_fd, wc.offset, fd_offset, wc.size, strerror(-err),
		               err);
		goto err_out_exit;
	}
		...
		if (++c->last_read_index >= (int)ARRAY_SIZE(c->last_reads)) {
			...
			if (old_ra != c->random_access) {
				DNET_LOG_ERROR(c->blog,
				               ...
				               old_ra, c->random_access, (unsigned long long)tmp,
				               (unsigned long long)c->vm_total);
			}
...
static int blob_del_range_callback(struct eblob_backend_config *c, struct eblob_range_request *req) {
	...
	DNET_LOG_DEBUG(c->blog, "%s: EBLOB: blob-read-range: DEL", dnet_dump_id_str(req->record_key));

	memcpy(key.id, req->record_key, EBLOB_ID_SIZE);
	err = eblob_remove(req->back, &key);
	if (err) {
		DNET_LOG_DEBUG(c->blog, "%s: EBLOB: blob-read-range: DEL: err: %d", dnet_dump_id_str(req->record_key),
		               err);
	}
...
static int blob_range_callback(struct eblob_range_request *req) {
	...
	DNET_LOG_NOTICE(p->blog, "%s: EBLOB: blob-range: limit: %llu [%llu, %llu]: start: %s, end: %s: io "
	                         "record/requested: offset: %llu/%llu, size: %llu/%llu",
	                cur_id, (unsigned long long)req->current_pos, (unsigned long long)req->requested_limit_start,
	                (unsigned long long)req->requested_limit_num, start_id, end_id,
	...
	if (p->keys_size == p->keys_cnt) {
		...
		if (p->keys == NULL) {
			err = -ENOMEM;
			DNET_LOG_ERROR(p->blog,
			               "%s: EBLOB: blob-del-range: can't (re-)allocate memory, new size: %" PRIu64 "",
			               cur_id, p->keys_size);
			goto err_out_exit;
		}
	}
	...
	DNET_LOG_DEBUG(p->blog, "%s: count: %llu", cur_id, (unsigned long long)(p->keys_cnt));
	p->keys_cnt++;
...
static int blob_read_range(struct eblob_backend_config *c, void *state, struct dnet_cmd *cmd, void *data) {
	...
	if (err) {
		DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-read-range: %d: %s", dnet_dump_id_str(io->id), err,
		               strerror(-err));
		goto err_out_exit;
	}

	if ((cmd->cmd == DNET_CMD_READ_RANGE) && (cmd->flags & DNET_ATTR_SORT)) {
		DNET_LOG_DEBUG(c->blog, "Sorting keys before sending");
	...
	for (i = start_from; i < p.keys_cnt; ++i) {
		...
		case DNET_CMD_READ_RANGE:
			if ((io->num > 0) && (i >= (io->num + start_from)))
				break;
			DNET_LOG_DEBUG(c->blog, "%s: EBLOB: blob-read-range: READ",
			               dnet_dump_id_str(p.keys[i].record_key));
			err = blob_read_range_callback(&p.keys[i]);
			break;
		case DNET_CMD_DEL_RANGE:
			DNET_LOG_DEBUG(c->blog, "%s: EBLOB: blob-read-range: DEL",
			               dnet_dump_id_str(p.keys[i].record_key));
		...
		if (err) {
			DNET_LOG_DEBUG(c->blog, "%s: EBLOB: blob-read-range: err: %d",
...
static int blob_del(struct eblob_backend_config *c, struct dnet_cmd *cmd) {
	...
	if (err) {
		DNET_LOG_ERROR(c->blog, "%s: EBLOB: blob-del: REMOVE: %d: %s", dnet_dump_id_str(cmd->id.id), err,
		               strerror(-err));
	}
...
static int blob_defrag_start(void *priv, enum dnet_backend_defrag_level level) {
	...
	case DNET_BACKEND_DEFRAG_COMPACT:
		...
		break;
	default:
		DNET_LOG_ERROR(c->blog, "DEFRAG: unknown defragmentation level: %d", (int)level);
		return -ENOTSUP;
	}
...
static int dnet_blob_set_sync(struct dnet_config_backend *b, const char *key __unused, const char *value) {
...
static int dnet_blob_set_data(struct dnet_config_backend *b, const char *key __unused, const char *file) {
	struct eblob_backend_config *c = b->data;
	int err;
...
static int dnet_blob_set_datasort_dir(struct dnet_config_backend *b, const char *key __unused, const char *dir) {
	struct eblob_backend_config *c = b->data;
	struct stat st;
	int err;
...
static int dnet_blob_set_index_block_size(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int
dnet_blob_set_index_block_bloom_length(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_periodic_timeout(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_records_in_blob(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
	uint64_t val = strtoul(value, NULL, 0);
...
static int dnet_blob_set_defrag_timeout(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_defrag_time(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_defrag_splay(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_defrag_percentage(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_blob_flags(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_blob_set_backend_id(struct dnet_config_backend *b, const char *key __unused, const char *value) {
	struct eblob_backend_config *c = b->data;
...
static int dnet_eblob_iterator(struct dnet_iterator_ctl *ictl,
	...
	int err;
	const int no_meta =
	        ireq->flags & DNET_IFLAGS_NO_META && !(ireq->flags & (DNET_IFLAGS_TS_RANGE | DNET_IFLAGS_DATA));
...
static int dnet_blob_config_init(struct dnet_config_backend *b, enum dnet_log_level level) {
	struct eblob_backend_config *c = b->data;
	...
	if (!c->data.file) {
		DNET_LOG_ERROR(c->blog, "blob: no data file present. Exiting");
		err = -EINVAL;
		goto err_out_exit;
	}
	...
	if (err) {
		err = -err;
		DNET_LOG_ERROR(c->blog, "blob: could not create last-read lock: %d", err);
		goto err_out_exit;
	}
...
static struct dnet_config_backend dnet_eblob_backend = {
	...
	.set_verbosity		= dnet_blob_set_verbosity,
};
