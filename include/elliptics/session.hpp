...
public:
	 ...
	 */
	async_read_result
	read_data(const key &id, const std::vector<int> &groups, const dnet_io_attr &io, unsigned int cmd);
	/*!
	 * \overload read_data(const key &id, const std::vector<int> &groups, const dnet_io_attr &io)
	 ...
	 */
	async_write_result
	write_data(const key &id, const data_pointer &file, uint64_t remote_offset, uint64_t chunk_size);
	 ...
	 * Returns async_write_result.
	 */
	async_write_result write_cas(const key &id,
	 ...
	 */
	async_write_result
	write_cas(const key &id, const argument_data &file, const dnet_id &old_csum, uint64_t remote_offset);
	 ...
	 */
	async_write_result
	write_prepare(const key &id, const argument_data &file, uint64_t remote_offset, uint64_t psize);
	...
	/*!
	 * Writes data \a file by the key \a id and remote offset \a remote_offset and commit key \a id data by \a
	 * csize.
	 *
	 * Returns async_write_result.
	 *
	 ...
	 */
	async_write_result
	write_commit(const key &id, const argument_data &file, uint64_t remote_offset, uint64_t csize);
	...
	set_backend_ids(const address &addr, uint32_t backend_id, const std::vector<dnet_raw_id> &ids);
	 ...
	 */
	async_iterator_result start_iterator(const key &id,
	 ...
	 */
	async_iterator_result start_copy_iterator(const key &id,
	                                          const std::vector<dnet_iterator_range> &ranges,
	                                          uint64_t flags,
	 ...
	 */
	async_iterator_result
	server_send(const std::vector<std::string> &keys, uint64_t flags, const std::vector<int> &groups);
	async_iterator_result
	server_send(const std::vector<dnet_raw_id> &ids, uint64_t flags, const std::vector<int> &groups);
	 ...
	 * \overload
	 */
	async_set_indexes_result
	set_indexes(const key &id, const std::vector<std::string> &indexes, const std::vector<data_pointer> &data);
	/*!
	 ...
	 */
	async_set_indexes_result
	update_indexes(const key &id, const std::vector<std::string> &indexes, const std::vector<data_pointer> &data);
	/*!
