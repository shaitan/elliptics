...
int dnet_update_notify(struct dnet_net_state *st, struct dnet_cmd *cmd, void *data)
{
	...
	list_for_each_entry(nt, &b->notify_list, notify_entry) {
		...
		dnet_send_reply(nt->state, &nt->cmd, &notif, sizeof(struct dnet_io_notification), 1, /*context*/ NULL);
	}
...
int dnet_notify_remove(struct dnet_net_state *st, struct dnet_cmd *cmd)
{
	...
	list_for_each_entry_safe(e, tmp, &b->notify_list, notify_entry) {
		...
		e->cmd.flags = 0;
		err = dnet_send_reply(e->state, &e->cmd, NULL, 0, 0, /*context*/ NULL);
...
int dnet_notify_init(struct dnet_node *n)
{
	...
	if (!n->notify_hash) {
		...
		DNET_ERROR(n, "Failed to allocate %u notify hash buckets", n->notify_hash_size);
		err = -ENOMEM;
		goto err_out_exit;
	}
	...
	dnet_log(n, DNET_LOG_INFO, "Successfully initialized notify hash table (%u entries)", n->notify_hash_size);
