...
int dnet_work_pool_alloc(struct dnet_work_pool_place *place,
	...
	pool->request_queue = dnet_request_queue_create(n);
...
again:
	...
	if (st->rcv_flags & DNET_IO_CMD) {
		...
		dnet_log(n, DNET_LOG_DEBUG, "%s: %s: received trans: %llu <- %s/%d: "
		         ...
		         dnet_dump_id(&c->id), dnet_cmd_string(c->cmd), tid, dnet_state_dump_addr(st), c->backend_id,
...
static void dnet_check_work_pool_place(struct dnet_work_pool_place *place,
                                       uint64_t *queue_size,
...
static int dnet_check_io(struct dnet_io *io) {
	uint64_t queue_size = 0, threads_count = 0;
