...
int dnet_version_check(struct dnet_net_state *st, int *version)
{
	...
	if ((version[0] == ELLIPTICS_PROTOCOL_VERSION_0) && (version[1] == ELLIPTICS_PROTOCOL_VERSION_1)) {
		dnet_log(n, DNET_LOG_INFO, "%s: reverse lookup command: network version: %d.%d.%d.%d, local version: %d.%d.%d.%d",
				dnet_state_dump_addr(st),
				version[0], version[1], version[2], version[3],
				ELLIPTICS_PROTOCOL_VERSION_0, ELLIPTICS_PROTOCOL_VERSION_1,
				ELLIPTICS_PROTOCOL_VERSION_2, ELLIPTICS_PROTOCOL_VERSION_3);
	} else {
